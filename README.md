...
- **多密钥轮询池**：round-robin 负载均衡，401/429/432/433 自动故障转移并停用问题密钥（429 按 retry-after 倒计时短期停用；432 套餐额度耗尽禁用到下次重置；433/401 永久停用），额度恢复后自动重新启用，也可在面板手动操作。停用状态持久化到 config.json，容器重启后恢复
- **实时额度查询**：使用官网 appSession token 调用内部 `/api/account`，绕开公共 `/usage` 接口的计费周期滞后问题，每 6 小时自动续期；同一账户多个 key 共享一次查询结果，减少官网请求
